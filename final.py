...
def refresh_table(filtered=None):
    ...
    for contact in data:
        emoji = {"favourite": "⭐", "blocked": "🚫"}.get(contact["status"], "🙂")
        tree.insert("", "end", values=(contact["name"], contact["phone"], contact["email"], emoji))
...
def show_contact_book():
    ...
    welcome_label = ctk.CTkLabel(welcome_frame, text=f"Welcome, {current_user.capitalize()}!", font=("Arial", 24))
...
def login_screen():
    ...
    # Image
    try:
        image = Image.open("contactbook_image.png").resize((100, 100))
        login_img = ImageTk.PhotoImage(image)
        label = tk.Label(frame, image=login_img, bg="#1a1a1a")
        label.image = login_img
        label.pack(pady=10)
    except:
