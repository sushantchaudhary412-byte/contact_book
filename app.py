...
def search_contacts(query):
    query = query.lower()
    filtered = [c for c in contacts if query in c["name"].lower() or query in c["phone"] or query in c["email"].lower()]
...
tk.Button(search_frame, text="Search", command=lambda: search_contacts(search_var.get())).pack(side=tk.LEFT, padx=5)
tk.Button(search_frame, text="Clear", command=lambda: [search_var.set(""), refresh_table()]).pack(side=tk.LEFT)
